...
def checkout_success(request, ticket_number):
    """
    ...
    messages.success(request, f'Ticket order successfully processed! \
        Your order number is {ticket_number}. A confirmation \
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        ticket_order_form = TicketOrderForm(form_data)
        if ticket_order_form.is_valid():
            ...
            return redirect(reverse('checkout_success', args=[order.ticket_number]))
        else:
    ...
    if request.user.is_authenticated:
        try:
            profile = UserProfile.objects.get(user=request.user)
            order_form = TicketOrderForm(initial={
